...
%RUN_FIG3_NMSE_NRF  Fig. 3 -- NMSE vs N_RF (hybrid compression sweep).
...
NMSE_db = nan(n_meth,n_nrf);  NMSE_std = nan(n_meth,n_nrf);

fprintf('=== Fig.3: NMSE vs N_RF  (SNR=%ddB, N_MC=%d) ===\n', SNR_fix, P.N_MC);

for ni = 1:n_nrf
    P.N_RF = NRF_vec(ni);
    fprintf('N_RF = %2d  ... ', P.N_RF);
    ...
    save_results_csv(CSV,'Fig3','NMSE_vs_NRF','compression_sweep', ...
        P.M,NRF_vec(ni),P.N,P.d,SNR_fix,'N_RF',NRF_vec(ni),'exact_USW', ...

...
for di = 1:n_d
    ...
    CRB_theta(di) = nanmedian(crb_th_mc);   % median robust to ill-cond outliers
...
% Legend southoutside; all 7 entries in a single row
leg8 = legend('Orientation','horizontal','NumColumns',4, ...
              'Location','southoutside','FontSize',8);
leg8.Title.String   = 'Filled: compressed | Open: full-array';
leg8.Title.FontSize = 8;
% Height 9.5 cm (+2 cm) for below-axes legend
nf_export_fig(fig8, 'fig8_vard', 'double', 'Height', 9.5);
...
nf_export_fig(fig8b, 'fig8b_vard_rmse', 'double', 'Height', 8.5);
